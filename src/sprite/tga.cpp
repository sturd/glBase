...
TGAImg::TGAImg()
{ 
	pImage = pPalette = pData = pTemp = NULL;
	iFrmCnt = iWidth = iHeight = iBPP = bEnc = 0;
	lImageSize = ByteCount = 0;
}

// Alternate constructor, used when generating temporary
// instance for splitting frames.  By Craig Sturdy.
TGAImg::TGAImg( short int Width, short int Height, short int BPP )
{
	iWidth = Width;
	iHeight = Height;
	iBPP = BPP * 8;
...
int TGAImg::Load( char* szFilename, int FrameCount )
{
	iFrmCnt = FrameCount;
	using namespace std;
	ifstream fIn;
...
// Custom routine to isolate, regions of main image
// depending upon frame required.
// Written by Craig Sturdy 16/02/09
unsigned char* TGAImg::GetFrame( int Frame )
{
	// Calculate size of frame required
	int frmWidth = ( iWidth / iFrmCnt ) * GetBPP();

	// Determine start point for reading main image
	int frmStrt = ( frmWidth *  Frame );

	// initialise *pTemp pointer, setting the array
	// size to match the calculated dimensions of the
	// required frame
	pTemp = new unsigned char[ frmWidth * iHeight ];
	...
	{
		pTemp[ ( i * frmWidth ) + j ] = 
			pImage[ frmStrt + ( iWidth * GetBPP() * i ) + j ];
	}
...
void TGAImg::SetImg( unsigned char *Raw )
{
	pImage = Raw;
}
